...
(function () {
  ...
  function initFilters() {
    ...
    btns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        items.forEach((it) => {
          const show = f === "all" ? !it.dataset.hideOnAll : it.dataset.cat === f;
  ...
  function initLightbox() {
    const triggers = $$("[data-lightbox], [data-video-src]");
    ...
    triggers.forEach((t) => {
      t.addEventListener("click", () => {
        const videoSrc = t.dataset.videoSrc;
        const title = t.dataset.lightbox || t.dataset.videoTitle || "";
        if (videoSrc) {
          body.innerHTML = `<video src="${videoSrc}" controls autoplay style="max-width:100%; max-height:80vh; outline:none; border-radius: var(--radius-sm);"></video>`;
        } else {
    ...
    // Auto-play video from query parameter (e.g., ?play=led-backdrop)
    const params = new URLSearchParams(location.search);
    const playId = params.get("play");
    if (playId) {
      const targetCard = document.getElementById(playId);
      if (targetCard) {
        setTimeout(() => {
          targetCard.scrollIntoView({ behavior: "smooth", block: "center" });
          targetCard.click();
        }, 800); // Allow entry animations and page layout to settle
      }
    }
  }
